...
void SeExprEdPreviewWidget::makePreview(const QString& path)
{
    QFileInfo fi( path );
...
SeExprEdPreviewWidget::SeExprEdPreviewWidget( QWidget *parent )
    : QWidget( parent )
{
    _pm = new QLabel( this );
...
SeExprEdFileDialog::SeExprEdFileDialog(QWidget* parent) : QFileDialog(parent)
{
...
void SeExprEdFileDialog::handleOk()
{
...
void SeExprEdFileDialog::editReturnPress()
{
    if (!_nameEdit) return;
...
void SeExprEdFileDialog::addFavoritesButton(QString dirname, QString linkname,
    QString linkdir)
{
...
void SeExprEdFileDialog::gotoFavorites()
{
...
QString SeExprEdFileDialog::getOpenFileName( const QString& caption,
...
QStringList SeExprEdFileDialog::getOpenFileNames( const QString& caption,
    const QString& startWith, const QString& filter )
{
    if ( !filter.isEmpty() )
    {
...
QString SeExprEdFileDialog::getExistingDirectory( const QString& caption,
    const QString& startWith, const QString& filter )
{
    if ( !filter.isEmpty() )
    {
...
QString SeExprEdFileDialog::getExistingOrNewDirectory( const QString& caption,
    const QString& startWith, const QString& filter )
{
    _createDir = 1;
...
QString SeExprEdFileDialog::getSaveFileName( const QString& caption,
    const QString& startWith, const QString& filter )
{
    if ( !filter.isEmpty() )
    {
...
void SeExprEdFileDialog::setPreview()
{
    ...
    _pw = new SeExprEdPreviewWidget( this );
...
void SeExprEdFileDialog::addComboBox(QString s, QStringList sl)
{
    QGridLayout *layout = findChild<QGridLayout *>("gridLayout");
    if (!layout) return;
...
void SeExprEdFileDialog::selChanged(const QString& path)
{
